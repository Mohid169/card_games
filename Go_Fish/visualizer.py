...
class GameVisualizer:
    ...
    def draw_player_hand(self, player, index):
        ...
        for i, card in enumerate(player.hand):
            ...
            if card not in self.card_sprites:
                sprite = CardSprite(card, self.width//2, self.height//2)  # Start from deck
                self.card_sprites[card] = sprite
                self.all_sprites.add(sprite)
    ...
    def draw(self):
        ...
        # Draw deck
        deck_text = self.font.render(f"Deck: {len(self.game.deck.cards)}", True, self.BLACK)
        ...
        for current_player in game.players:
            ...
            if current_player.check_hand():
                if len(game.deck.cards) > 0:
                    ...
                    visualizer.set_message(f"{current_player.name} draws {len(cards_drawn)} cards")
